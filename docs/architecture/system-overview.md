...
运行链固定为 `Twist source → robot_control → HostMotionCommand (Platform API 5) → BridgeCore → upper v3 → firmware CommandManagement/SafetyManagement/MotionControl`。`robot_control` 是 `chassis/host_motion_command` 的唯一发布者，但只拥有 Host 子来源选择；固件拥有整车来源仲裁和物理运动许可。Bridge 只发布原始观测与链路/固件事实；`robot_state_estimation` 生成 `wheel/odom` 和 `imu/data`，`formal_odometry` 在兼容门允许时独占正式 `/odom` 与 `odom → base_link` TF。
