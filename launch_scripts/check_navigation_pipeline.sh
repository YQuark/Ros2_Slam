...
print_topic_status "/chassis/host_motion_command" "Host 候选底盘命令"
echo
...
if ! ros2_lifecycle_is_active "/bt_navigator"; then
    ...
    echo "  5. ros2 topic hz /chassis/host_motion_command"
fi
echo
...
echo "  • /cmd_vel/nav 有消息但 /chassis/host_motion_command 没有: 查 robot_control"
echo "  • Host 候选有消息但车不动: 查 link/firmware control state、串口、底盘运动许可和 /odom"
