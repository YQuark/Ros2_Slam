"""Pure four-wheel motion consistency supervisor."""
...
class SupervisorConfig:
    ...
    covariance_max_multiplier: float = 10.0
    ...
    def update(
        ...
        *,
        ...
        if not c.enabled:
            ...
            return SupervisorResult(0.0, self.level, 1.0, 1.0, False, "disabled")
        pair = max(abs(wheel_speeds[0] - wheel_speeds[1]), abs(wheel_speeds[2] - wheel_speeds[3]))
        pair_score = _normalize(pair, c.wheel_pair_warn_mps, c.wheel_pair_critical_mps)
        tracking = 0.0
        ...
        tracking_score = _normalize(tracking, c.tracking_warn_mps, c.tracking_critical_mps)
        yaw_score = (
            0.0
            if gyro_z is None
        ...
        unexpected = (
            abs(command_vx) < c.zero_command_linear_mps
        ...
        components: Dict[str, float] = {
            "wheel_pair": pair_score,
            "tracking": tracking_score,
            "yaw": yaw_score,
            "unexpected_motion": 1.0 if unexpected else 0.0,
        }
        ...
        covariance = min(c.covariance_max_multiplier, 1.0 + 9.0 * score * score)
        return SupervisorResult(
            score, self.level, scale, covariance, self.level is SupervisorLevel.CRITICAL, reason
        )
